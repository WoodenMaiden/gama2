/*******************************************************************************************************
 *
 * DXFTextParser.java, in gama.dependencies, is part of the source code of the
...
public class DXFTextParser {
	 ...
	 */
	public static TextDocument parseDXFMText(final DXFMText text) {
		// initialize
		...
		if (formatting) { parseStyledTextParagraphSettings(key, value.toString(), p); }
		...
		if (doc.getParagraphCount() == 0) { doc.addStyledParagraph(p); }
	 ...
	 */
	protected static StyledTextParagraph createParagraphFromParent(final StyledTextParagraph parent) {
		StyledTextParagraph p = new StyledTextParagraph();
	 ...
	 * @return the text document
	 */
	public static TextDocument parseDXFText(final DXFText text) {
		TextDocument doc = new TextDocument();
		...
		if (marker == 1 || marker == 3) { buf.append('%'); }
	 ...
	 * @param para the para
	 */
	public static void parseStyledTextParagraphSettings(final char key, final String value,
			final StyledTextParagraph para) {
		switch (key) {
			case 'A':
	 ...
	 * @param para the para
	 */
	public static void parseFontSettings(final String value, final StyledTextParagraph para) {
	 ...
	 * @return the string
	 */
	public static String parseSymbols(final String text) {
		boolean asciicontrol = false;
		StringBuilder buf = new StringBuilder();
		int marker = 0;
		char c;
