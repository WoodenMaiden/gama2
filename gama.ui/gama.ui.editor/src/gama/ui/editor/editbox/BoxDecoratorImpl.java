/*******************************************************************************************************
 *
 * BoxDecoratorImpl.java, in gama.ui.editor, is part of the source code of the GAMA modeling and simulation
 * platform (v.1.9.0).
 *
...
public class BoxDecoratorImpl implements IBoxDecorator {
	...
	@Override
	public IBoxProvider getProvider() { return provider; }

	@Override
	public void setProvider(final IBoxProvider newProvider) { this.provider = newProvider; }
	...
	@Override
	public void setStyledText(final StyledText newSt) { this.boxText = newSt; }
	...
	void drawBackgroundBoxes() {
		...
		if (settings.getNoBackground() && oldBackground != null) { bc = new Color(null, oldBackground); }
		if (bc != null) {
			final Rectangle rec = newImage.getBounds();
			fillRectangle(bc, gc, rec.x, rec.y, rec.width, rec.height);
		}

		if (settings.getAlpha() > 0) { gc.setAlpha(settings.getAlpha()); }
		...
		for (final Box b : visibleBoxes) {
			if (!b.isOn) {
				drawRect(gc, b, b.rec.x - xOffset, b.rec.y - yOffset, settings.getExpandBox() ? r0.width : b.rec.width,
						b.rec.height);
			}
		}
	...
	void fillRectangle(final Color c, final GC gc, final int x, final int y, final int width, final int height) {
		...
		} else if (settings.getFillGradient() && settings.getFillGradientColor() != null) {
	...
	public void decorate(final boolean mouseDbClickColorChange) {
		...
		if (mouseDbClickColorChange) { boxMouseClick = new BoxMouseClickListener(); }

		final Color c = boxText.getBackground();
		if (c != null) { oldBackground = c.getRGB(); }
		oldIndent = boxText.getIndent();
		if (oldIndent < 3) { boxText.setIndent(3); }
		...
		if (mouseDbClickColorChange) { boxText.addMouseListener(boxMouseClick); }
	...
	public void undecorate() {
		...
		if (settingsChangeListener != null) { settings.removePropertyChangeListener(settingsChangeListener); }
		if (boxText == null || boxText.isDisposed()) return;
		decorated = false;
		if (boxMouseClick != null) { boxText.removeMouseListener(boxMouseClick); }
		if (boxTextChange != null) { boxText.getContent().removeTextChangeListener(boxTextChange); }
		if (boxMouseTrack != null) { boxText.removeMouseTrackListener(boxMouseTrack); }
		if (boxMouseMove != null) { boxText.removeMouseMoveListener(boxMouseMove); }
		if (boxPaint != null) { boxText.removePaintListener(boxPaint); }
		if (fillMouseClick != null) { boxText.removeMouseListener(fillMouseClick); }
		if (boxModify != null) { boxText.removeModifyListener(boxModify); }
		if (boxKey != null) { boxText.removeKeyListener(boxKey); }
	...
	protected Collection<Box> visibleBoxes() {
		...
		for (final Box b : boxes) { if (b.intersects(start, end)) { result.add(b); } }

		final int yOffset = boxText.getTopPixel();
		final int xOffset = boxText.getHorizontalPixel();
		for (final Box b1 : result) {
			if (b1.rec == null) {
				final Point s = boxText.getLocationAtOffset(b1.start);
				if (b1.tabsStart > -1 && b1.tabsStart != b1.start) {
					final Point s1 = boxText.getLocationAtOffset(b1.tabsStart);
					if (s1.x < s.x) { s.x = s1.x; }
				}
				final Point e = boxText.getLocationAtOffset(b1.end);
				if (b1.end != b1.maxEndOffset) {
					final Point e1 = boxText.getLocationAtOffset(b1.maxEndOffset);
					e.x = e1.x;
				}
				final Rectangle rec2 = new Rectangle(s.x + xOffset - 2, s.y + yOffset - 1, e.x - s.x + 6,
						e.y - s.y + boxText.getLineHeight(b1.end));
				b1.rec = rec2;
				updateWidth(b1);
				updateWidth3(b1);
			}
	...
	protected boolean turnOnAll(final int x, final int y) {
		...
		for (final Box b : visibleBoxes()) {
			if (contains(b.rec, x, y)) {
				...
				if (newCurrent == null || newCurrent.offset < b.offset) { newCurrent = b; }
			} else if (b.isOn) {
				b.isOn = false;
				redraw = true;
			}
		}
		if (!redraw) { redraw = newCurrent != currentBox; }
	...
	protected boolean turnOnOne(final int x, final int y) {
		Box newCurrent = null;
		for (final Box b : visibleBoxes()) {
			if (contains(b.rec, x, y)) { newCurrent = b; }
			b.isOn = false;
		}
		if (newCurrent != null) { newCurrent.isOn = true; }
	...
	/**
	 * The listener interface for receiving boxModify events. The class that is interested in processing a boxModify
	 * event implements this interface, and the object created with that class is registered with a component using the
	 * component's <code>addBoxModifyListener<code> method. When the boxModify event occurs, that object's appropriate
	 * method is invoked.
	 *
	 * @see BoxModifyEvent
	 */
	...
	/**
	 * The listener interface for receiving boxKey events. The class that is interested in processing a boxKey event
	 * implements this interface, and the object created with that class is registered with a component using the
	 * component's <code>addBoxKeyListener<code> method. When the boxKey event occurs, that object's appropriate method
	 * is invoked.
	 *
	 * @see BoxKeyEvent
	 */
	...
	/**
	 * The listener interface for receiving settingsChange events. The class that is interested in processing a
	 * settingsChange event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addSettingsChangeListener<code> method. When the settingsChange event
	...
	/**
	 * The listener interface for receiving boxPaint events. The class that is interested in processing a boxPaint event
	 * implements this interface, and the object created with that class is registered with a component using the
	 * component's <code>addBoxPaintListener<code> method. When the boxPaint event occurs, that object's appropriate
	 * method is invoked.
	 *
	 * @see BoxPaintEvent
	 */
	...
	/**
	 * The listener interface for receiving boxMouseMove events. The class that is interested in processing a
	 * boxMouseMove event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addBoxMouseMoveListener<code> method. When the boxMouseMove event occurs,
	...
	class BoxMouseMoveListener implements MouseMoveListener {
		...
		public void mouseMove(final MouseEvent e) {
			...
			if (turnOnBox(e.x, e.y)) { drawBackgroundBoxes(); }
		}
	}

	/**
	 * The listener interface for receiving boxMouseTrack events. The class that is interested in processing a
	 * boxMouseTrack event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addBoxMouseTrackListener<code> method. When the boxMouseTrack event occurs,
	 * that object's appropriate method is invoked.
	 *
	...
	/**
	 * The listener interface for receiving boxTextChange events. The class that is interested in processing a
	 * boxTextChange event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addBoxTextChangeListener<code> method. When the boxTextChange event occurs,
	 * that object's appropriate method is invoked.
	 *
	...
	/**
	 * The listener interface for receiving boxMouseClick events. The class that is interested in processing a
	 * boxMouseClick event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addBoxMouseClickListener<code> method. When the boxMouseClick event occurs,
	 * that object's appropriate method is invoked.
	 *
	...
	class BoxMouseClickListener extends MouseAdapter {
		...
		public void mouseDoubleClick(final MouseEvent e) {
			...
			int level = -1;
			for (final Box b : visibleBoxes()) { if (contains(b.rec, x, y) && level < b.level) { level = b.level; } }
			level++;

			final ColorDialog colorDialog = new ColorDialog(boxText.getShell());
			final Color oldColor1 = settings.getColor(level);
			if (oldColor1 != null) { colorDialog.setRGB(oldColor1.getRGB()); }
	...
	class FillBoxMouseClick extends MouseAdapter {
		...
		public void mouseDown(final MouseEvent e) {
			...
			Box fillBox = null;
			for (final Box b : visibleBoxes()) { if (contains(b.rec, x, y)) { fillBox = b; } }
