/*******************************************************************************************************
 *
 * ImageHelper.java, in gaml.extension.image, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
public class ImageHelper implements ImageConstants {
	...
	static GamaImage apply(GamaImage src, final BufferedImageOp op) {
		final var type = src.getType();
		if (type != BufferedImage.TYPE_INT_RGB && type != BufferedImage.TYPE_INT_ARGB) {
			src = copyToOptimalImage(src);
		}
		final var bounds = op.getBounds2D(src);
	...
	protected static GamaImage copyToOptimalImage(final Image src) {
		...
		final var type = src instanceof final BufferedImage bu && bu.getTransparency() == Transparency.OPAQUE
				? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB;
		final var result = GamaImage.ofDimensions(src.getWidth(null), src.getHeight(null), type);
	...
	static GamaImage rotate(final GamaImage src, final int typeOfRotation)
		...
		var newWidth = src.getWidth();
		var newHeight = src.getHeight();
		...
		final var result = GamaImage.bestFor(src, newWidth, newHeight);
		final var g2d = result.createGraphics();
		g2d.setRenderingHints(ImageConstants.HINTS);
	...
	static GamaImage resize(final GamaImage src, final Mode resizeMode, int targetWidth, int targetHeight)
		...
		final var currentWidth = src.getWidth();
		final var currentHeight = src.getHeight();
		final var ratio = (float) currentHeight / (float) currentWidth;
	...
	static GamaImage scaleImageIncrementally(GamaImage src, final int targetWidth, final int targetHeight) {
		...
		var currentWidth = src.getWidth();
		var currentHeight = src.getHeight();
		do {
			final var prevCurrentWidth = currentWidth;
			final var prevCurrentHeight = currentHeight;
			...
			final var incrementalImage = scaleImage(src, currentWidth, currentHeight);
	...
	public static GamaImage scaleImage(final Image bufferedImage, final int targetWidth, final int targetHeight) {
		final var result = GamaImage.bestFor(bufferedImage, targetWidth, targetHeight);
		final var resultGraphics = result.createGraphics();
	...
	public static void flipImageVertically(final GamaImage image) {
		...
		for (var i = 0; i < image.getHeight() / 2; i++) {
	...
	record TransferableImage(Image i) implements Transferable {
		...
		public DataFlavor[] getTransferDataFlavors() {
			final var flavors = new DataFlavor[1];
		...
		public boolean isDataFlavorSupported(final DataFlavor flavor) {
			final var flavors = getTransferDataFlavors();
			for (final DataFlavor dataFlavor : flavors) { if (flavor.equals(dataFlavor)) { return true; } }
			return false;
		}
	}
