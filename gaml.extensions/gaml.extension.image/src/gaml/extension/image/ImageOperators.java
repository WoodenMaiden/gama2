/*******************************************************************************************************
 *
 * ImageOperators.java, in gaml.extension.image, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
import static gaml.extension.image.ImageHelper.apply;
import static gaml.extension.image.ImageHelper.resize;
import static gaml.extension.image.ImageHelper.rotate;
...
import gaml.extension.image.ImageHelper.Mode;
import gaml.extension.image.ImageHelper.TransferableImage;
...
public class ImageOperators implements ImageConstants {
	...
	public static GamaImage snapshot(final IScope scope, final IAgent exp, final String displayName) {
		...
		if (exp instanceof final ITopLevelAgent top) {
		...
		if (!(output instanceof final LayeredDisplayOutput ldo)) return null;
		final var surface = ldo.getSurface();
	...
	public static GamaImage grayscale(final IScope scope, final GamaImage image) {
		try {
			return apply(image, ImageConstants.OP_GRAYSCALE);
	...
	public static GamaImage darker(final IScope scope, final GamaImage image) {
		try {
			return apply(image, ImageConstants.OP_DARKER);
		} catch (final Exception e) {
			return image;
		}
	}
	...
	public static GamaImage brigther(final IScope scope, final GamaImage image) {
		try {
			return apply(image, ImageConstants.OP_BRIGHTER);
		} catch (final Exception e) {
			return image;
		}
	}
	...
	public static GamaImage antialiased(final IScope scope, final GamaImage image) {
		try {
			return apply(image, ImageConstants.OP_ANTIALIAS);
		} catch (final Exception e) {
			return image;
		}
	}
	...
	public static GamaImage scaled_by(final IScope scope, final GamaImage image, final Double scale) {
		...
		final var newWidth = (int) Math.round(image.getWidth() * scale);
		final var newHeight = (int) Math.round(image.getHeight() * scale);
	...
	public static GamaImage rotated(final IScope scope, final GamaImage image, final double angleInDegrees) {
		final var angle = Math.abs(angleInDegrees) % 360 * Math.signum(angleInDegrees);
		...
		final var rotated = GamaImage.ofDimensions(newWidth, newHeight, BufferedImage.TYPE_INT_ARGB);
		final var g2 = rotated.createGraphics();
		...
		g2.setRenderingHints(ImageConstants.HINTS);
	...
	public static GamaImage tint(final IScope scope, final GamaImage image, final GamaColor color) {
		final var tintedSprite = GamaImage.ofDimensions(image.getWidth(), image.getHeight(), Transparency.TRANSLUCENT);
		final var graphics = tintedSprite.createGraphics();
		graphics.drawImage(image, 0, 0, null);
		graphics.dispose();
		final var cm = tintedSprite.getColorModel();
		final var raster = tintedSprite.getRaster();
		final var r = color.getRed() / 255f;
		final var g = color.getGreen() / 255f;
		final var b = color.getBlue() / 255f;
		final var a = color.getAlpha() / 255f;
		for (var i = 0; i < tintedSprite.getWidth(); i++) {
			for (var j = 0; j < tintedSprite.getHeight(); j++) {
				var ax = cm.getAlpha(raster.getDataElements(i, j, null));
				var rx = cm.getRed(raster.getDataElements(i, j, null));
				var gx = cm.getGreen(raster.getDataElements(i, j, null));
				var bx = cm.getBlue(raster.getDataElements(i, j, null));
				rx *= r;
				gx *= g;
				bx *= b;
				ax *= a;
	...
	public static GamaImage tint(final IScope scope, final GamaImage image, final GamaColor color, final double ratio) {
		final var w = image.getWidth();
		final var h = image.getHeight();
		final var dyed = GamaImage.ofDimensions(w, h, BufferedImage.TYPE_INT_ARGB);
		final var g = dyed.createGraphics();
	...
	public static GamaImage blend(final IScope scope, final GamaImage image, final GamaImage overlay,
			final double ratio) {
		final var composed = ImageHelper.copyToOptimalImage(image);
		final var g2d = composed.createGraphics();
		g2d.setComposite(AlphaComposite.SrcOver.derive(Math.min(1f, Math.max((float) ratio, 0f))));
		final var x = (composed.getWidth() - overlay.getWidth()) / 2;
		final var y = (composed.getHeight() - overlay.getHeight()) / 2;
	...
	public static GamaImage blur(final IScope scope, final GamaImage image) {
		return apply(image, ImageConstants.OP_BLUR);
	}
	...
	public static GamaImage sharpen(final IScope scope, final GamaImage image) {
		return apply(image, ImageConstants.OP_SHARPEN);
	}
	...
	public static GamaImage cropped(final IScope scope, final GamaImage image, final int ox, final int oy, final int ow,
			final int oh) {
		final var iw = image.getWidth();
		final var ih = image.getHeight();
		final var width = Math.min(iw, Math.max(0, ow));
		final var height = Math.min(ih, Math.max(0, oh));
		final var x = Math.min(iw, Math.max(0, ox));
		final var y = Math.min(ih, Math.max(0, oy));
		if (x == width || width == 0 || height == 0 || y == height) return image;
		if (x == 0 && y == 0 && width == iw && height == ih) return image;
		final var result = GamaImage.bestFor(image, width, height);
		final var g = result.getGraphics();
	...
	public static GamaImage image(final int w, final int h, final GamaColor color) {
		final var gi = GamaImage.ofDimensions(w, h, BufferedImage.TYPE_INT_ARGB);
	...
	public static IMatrix matrix(final IScope scope, final GamaImage image) {
		final var xSize = image.getWidth();
		final var ySize = image.getHeight();
		final IMatrix matrix = new GamaIntMatrix(xSize, ySize);
		for (var i = 0; i < xSize; i++) {
			for (var j = 0; j < ySize; j++) { matrix.set(scope, i, j, image.getRGB(i, j)); }
		}
		return matrix;
	}
