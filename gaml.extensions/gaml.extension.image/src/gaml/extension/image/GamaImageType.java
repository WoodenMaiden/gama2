/*******************************************************************************************************
 *
 * GamaImageType.java, in gaml.extension.image, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
public class GamaImageType extends GamaType<GamaImage> {
	...
	public static GamaImage staticCast(final IScope scope, final Object obj, final boolean copy)
			throws GamaRuntimeException {
		if (obj instanceof final GamaImage im) {
			if (copy) { return GamaImage.from(im, im.getAlpha(scope)); }
			return im;
		}
		if (obj instanceof final GamaIntMatrix mat) { return GamaImage.from(scope, mat); }
		if (obj instanceof final BufferedImage im) { return GamaImage.from(im, true); }
		if (obj instanceof final Image im) { return ImageHelper.copyToOptimalImage(im); }
		if (obj instanceof final GamaImageFile f) {
			return GamaImage.from(f.getImage(scope, true), true, f.getOriginalPath());
		}
		if (obj instanceof final GamaPoint p) { return ImageOperators.image((int) p.getX(), (int) p.getY()); }
		if (obj instanceof final String s) { return staticCast(scope, new GamaImageFile(scope, s), false); }
		if (obj instanceof final GamaField f) { return GamaImage.from(scope, f); }
		if (obj instanceof final ISpecies s && s.isGrid()) {
	...
	public GamaImage copyFromClipboard(final IScope scope) {
		...
		final var content = ImageConstants.clipboard.getContents(null);
