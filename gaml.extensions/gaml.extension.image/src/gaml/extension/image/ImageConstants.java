/*******************************************************************************************************
 *
 * ImageConstants.java, in gaml.extension.image, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
public interface ImageConstants {
	...
	ConvolveOp OP_ANTIALIAS =
			new ConvolveOp(new Kernel(3, 3, new float[] { .0f, .08f, .0f, .08f, .68f, .08f, .0f, .08f, .0f }),
					ConvolveOp.EDGE_NO_OP, ImageConstants.HINTS);

	/**
	 * A {@link RescaleOp} used to make any input image 10% darker.
	 */
	RescaleOp OP_DARKER = new RescaleOp(0.9f, 0, ImageConstants.HINTS);

	/**
	 * A {@link RescaleOp} used to make any input image 10% brighter.
	 */
	RescaleOp OP_BRIGHTER = new RescaleOp(1.1f, 0, ImageConstants.HINTS);
