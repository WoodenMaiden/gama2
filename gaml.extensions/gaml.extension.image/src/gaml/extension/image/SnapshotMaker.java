/*******************************************************************************************************
 *
 * SnapshotMaker.java, in gaml.extension.image, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
public class SnapshotMaker implements ISnapshotMaker {
	...
	public void takeAndSaveSnapshot(final IDisplaySurface surface, final GamaPoint customDimensions) {
		...
		final var image = captureImage(surface, customDimensions);
		if (image == null) { return; }
		final var fileName = buildPath(scope, surface);
		try {
			...
			final var e = GamaRuntimeException.create(ex, scope);
	...
	private String buildPath(final IScope scope, final IDisplaySurface surface) {
		...
		final var autosavePath = surface.getData().getAutosavePath();
		String fileName;
		if (autosavePath == null || autosavePath.isBlank()) {
			final var snapshotFile = FileUtils.constructAbsoluteFilePath(scope, IDisplaySurface.SNAPSHOT_FOLDER_NAME
	...
	public GamaImage captureImage(final IDisplaySurface surface, final GamaPoint customDimensions) {
		...
		final var composite = surface.getBoundsForRobotSnapshot();
		final var width =
		...
		if (GamaPreferences.Displays.DISPLAY_FAST_SNAPSHOT.getValue() && robot != null) {
			try {
				...
				final Image im = robot.createScreenCapture(composite);
	...
	private GamaImage takeSnapshotOfChart(final ChartLayer chart, final int width, final int height) {
		...
		final var co = chart.getChart();
		if (co == null) { return null; }
		DEBUG.OUT("Chart is rendered on " + width + " " + height);
		final var im = GamaImage.ofDimensions(width, height, true);
		final var jfc = co.getJFChart();
		final var g2 = im.createGraphics();
	...
	public void takeAndSaveScreenshot(final IScope scope, final String autosavePath) {
		...
		String fileName;
		if (autosavePath == null || autosavePath.isBlank()) {
			final var snapshotFile = FileUtils.constructAbsoluteFilePath(scope,
		...
		GamaImage image = null;
		try {
			final var r = getScreenTotalArea();
			final Image im = robot.createScreenCapture(r);
	...
	public static SnapshotMaker getInstance() {
		if (SnapshotMaker.instance == null) { SnapshotMaker.instance = new SnapshotMaker(); }

		return SnapshotMaker.instance;
	}
	...
	static public Rectangle getScreenTotalArea() {
		...
		final var screens = ge.getScreenDevices();
		final var allScreenBounds = new Rectangle();
		for (final GraphicsDevice screen : screens) {
			final var screenBounds = screen.getDefaultConfiguration().getBounds();
