/*******************************************************************************************************
 *
 * Connector.java, in gaml.extension.network, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
import gaml.extension.network.skills.INetworkSkill;
...
public abstract class Connector implements IConnector {
		...
		synchronized (lockGroupManagment) {
			switch (action) {
				case FETCH_ALL_MESSAGE_THREAD_SAFE_ACTION: {
					...
					final var allMessage = this.receivedMessage;
					this.receivedMessage = newBox;
					return allMessage;
				}
				case PUSH_RECEIVED_MESSAGE_THREAD_SAFE_ACTION: {
					final var bb = this.boxFollower.get(groupName) == null ? this.boxFollower.get("ALL")
							: this.boxFollower.get(groupName);
					for (final IAgent agt : bb) {
						final var messages = receivedMessage.get(agt);
	...
	public void send(final IAgent sender, final String receiver, final GamaMessage content) {
		if (!this.forceNetworkUse && this.boxFollower.containsKey(receiver)) {
			...
			this.pushAndFetchthreadSafe(Connector.PUSH_RECEIVED_MESSAGE_THREAD_SAFE_ACTION, receiver, msg);
			// Fix for #3335
			return;
		}
		...
		if (!this.localMemberNames.containsKey(receiver)) {
			if (!isRaw()) {
				final var cmsg = new CompositeGamaMessage(sender.getScope(), content);
	...
	public Map<IAgent, LinkedList<ConnectorMessage>> fetchAllMessages() {
		return pushAndFetchthreadSafe(Connector.FETCH_ALL_MESSAGE_THREAD_SAFE_ACTION, null, null);
	}
	...
	public void leaveTheGroup(final IAgent agt, final String groupName) {
		...
		final var members = this.boxFollower.get(groupName);
		if (members != null) {
			members.remove(agt);
	...
	public void joinAGroup(final IAgent agt, final String groupName) {
		...
		var agentBroadcast = this.boxFollower.get(groupName);
	...
	public void connect(final IAgent agent) throws GamaNetworkException {
		final var netAgent = (String) agent.getAttribute(INetworkSkill.NET_AGENT_NAME);
