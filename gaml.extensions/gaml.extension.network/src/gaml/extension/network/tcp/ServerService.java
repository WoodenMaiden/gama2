/*******************************************************************************************************
 *
 * ServerService.java, in gaml.extension.network, is part of the source code of the GAMA modeling and simulation
 * platform (v.1.9.2).
 *
...
import gaml.extension.network.common.socket.IListener;
...
public class ServerService extends Thread implements SocketService, IListener {
			...
			try {
				...
				if (!clientSocket.isClosed() && !clientSocket.isInputShutdown()) {
					...
					if (list_net_agents != null && !list_net_agents.contains(clientSocket.toString())) {
						...
						final var cliThread = new ClientService(clientSocket, connector);
	...
	public void sendMessage(final String msg, final String receiver) throws IOException {
		...
		final var cliThread = (ClientService) myAgent.getAttribute(TCPConnector._TCP_CLIENT + receiver);
	...
	public void receivedMessage(final String sender, final String message) {
		...
		if (MessageType.COMMAND_MESSAGE.equals(mte)) {
			...
			final var r = ((TCPConnector) connector).isRaw() ? message : MessageFactory.unpackReceiverName(message);
