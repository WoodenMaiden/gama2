/*******************************************************************************************************
 *
 * ClientService.java, in gaml.extension.network, is part of the source code of the GAMA modeling and simulation
 * platform (v.1.9.2).
 *
...
public class ClientService extends Thread implements SocketService {
	...
	public void startService() throws UnknownHostException, IOException {
		if (socket == null) { socket = new Socket(this.server, this.port); }
		isAlive = true;
	...
	@Override
	public boolean isOnline() { return isAlive; }
	...
	public void receivedMessage(final String sender, final String message) {
		...
		if (MessageType.COMMAND_MESSAGE.equals(mte)) {
			...
			final var r = connector.isRaw() ? message : MessageFactory.unpackReceiverName(message);
