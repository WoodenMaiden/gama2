/*******************************************************************************************************
 *
 * MyArduino.java, in gaml.extension.network, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
...
public class MyArduino {
	...
	public boolean openConnection() {
		...
		final var alert = new AlertBox(new Dimension(400, 100), "Error Connecting", "Try Another port");
		alert.display();
		return false;
	}
	...
	public String serialRead() {
		...
		final var in = new Scanner(comPort.getInputStream());
		try {
	...
	public void serialWrite(final String s) {
		...
		THREADS.WAIT(5);
		final var pout = new PrintWriter(comPort.getOutputStream());
		pout.print(s);
		pout.flush();
	...
	public void serialWrite(final String s, final int noOfChars, final int delay) {
		...
		THREADS.WAIT(5);
		final var pout = new PrintWriter(comPort.getOutputStream());
		for (var i = 0; i < s.length(); i += noOfChars) {
