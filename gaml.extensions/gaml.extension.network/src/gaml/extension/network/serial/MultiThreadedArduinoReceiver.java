/*******************************************************************************************************
 *
 * MultiThreadedArduinoReceiver.java, in gaml.extension.network, is part of the source code of the GAMA modeling and
	...
	public void run() {
		...
		while (!closed) {
			...
			try {
				...
				final var sentence = arduino.serialRead(1);
				...
				final GamaMessage msg = new GamaMessage(myAgent.getScope(), "Arduino", myAgent.getName(), sentence);
