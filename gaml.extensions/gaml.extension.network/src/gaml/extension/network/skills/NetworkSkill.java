/*******************************************************************************************************
 *
 * NetworkSkill.java, in gaml.extension.network, is part of the source code of the GAMA modeling and simulation platform
 * (v.1.9.2).
 *
 ...
 *
 ********************************************************************************************************/
package gaml.extension.network.skills;
...
import gaml.extension.network.httprequest.HTTPRequestConnector;
import gaml.extension.network.mqtt.MQTTConnector;
import gaml.extension.network.serial.ArduinoConnector;
import gaml.extension.network.tcp.TCPConnector;
import gaml.extension.network.udp.UDPConnector;
import gaml.extension.network.websocket.WebSocketConnector;
...
public class NetworkSkill extends MessagingSkill {
	...
	public String systemExec(final IScope scope) {
		...
		final var commandToExecute = (String) scope.getArg("command", IType.STRING);
		...
		try {
			...
			final var stdError = new BufferedReader(new InputStreamReader(p.getErrorStream()));
	...
	public boolean connectToServer(final IScope scope) throws GamaRuntimeException {
		if (!scope.getExperiment().hasAttribute(NetworkSkill.REGISTRED_SERVER)) { this.startSkill(scope); }
		final IAgent agt = scope.getAgent();
		final var serverURL = (String) scope.getArg(INetworkSkill.SERVER_URL, IType.STRING);
		final var login = (String) scope.getArg(INetworkSkill.LOGIN, IType.STRING);
		final var password = (String) scope.getArg(INetworkSkill.PASSWORD, IType.STRING);
		final var networkName = (String) scope.getArg(INetworkSkill.WITHNAME, IType.STRING);
		final var protocol = (String) scope.getArg(INetworkSkill.PROTOCOL, IType.STRING);
		final var force_local = (Boolean) scope.getArg(INetworkSkill.FORCE_NETWORK_USE, IType.BOOL);
		final var raw_package = (Boolean) scope.getArg("raw", IType.BOOL);
		final var port = (Integer) scope.getArg(INetworkSkill.PORT, IType.INT);
		final var packet_size = (String) scope.getArg(INetworkSkill.MAX_DATA_PACKET_SIZE, IType.STRING);

		// Fix to Issue #2618
		final var serverKey = createServerKey(serverURL, port);

		final Map<String, IConnector> myConnectors = this.getRegisteredServers(scope);
		var connector = myConnectors.get(serverKey);
		if (connector == null) {
		...
		var serverList = (List<String>) agt.getAttribute(INetworkSkill.NET_AGENT_SERVER);
		if (serverList == null) {
	...
	public boolean registerToGroup(final IScope scope) {
		final IAgent agent = scope.getAgent();
		final var groupName = (String) scope.getArg(INetworkSkill.WITHNAME, IType.STRING);
		if (groupName != null) {
	...
	protected void effectiveSend(final IScope scope, final GamaMessage message, final Object receiver) {
		...
		var destName = receiver.toString();
		...
		final IAgent agent = scope.getAgent();
		final var serverNames = (List<String>) agent.getAttribute(INetworkSkill.NET_AGENT_SERVER);
	...
	protected List<IAgent> getRegisteredAgents(final IScope scope) {
		return (List<IAgent>) scope.getExperiment().getAttribute(NetworkSkill.REGISTERED_AGENTS);
	}
	...
	protected Map<String, IConnector> getRegisteredServers(final IScope scope) {
		return (Map<String, IConnector>) scope.getExperiment().getAttribute(NetworkSkill.REGISTRED_SERVER);
	}
	...
	private void initialize(final IScope scope) {
		...
		scope.getExperiment().setAttribute(NetworkSkill.REGISTERED_AGENTS, new ArrayList<IAgent>());
		scope.getExperiment().setAttribute(NetworkSkill.REGISTRED_SERVER, new HashMap<String, IConnector>());
	}
